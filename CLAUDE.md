...
10. **lib/agents/** - Multi-provider adapter seam (Issues #312 MP.1, #313 MP.2, #314 MP.3)
    ...
    - `codex.sh`: the pilot non-Claude adapter (#317, MP.6a) — the template for #318-#322. Build `codex exec [resume <id>] --json [-m <model>] <prompt>` (reuses `CLAUDE_MODEL`/`CLAUDE_USE_CONTINUE`, prepends loop context, never emits `--dangerously-bypass`); normalize maps Codex JSONL events → Ralph's internal struct (RALPH_STATUS in assistant text drives exit detection). Capabilities: `supports_token_usage`, `supports_session_resume` only — so the permission-denial CB and API-limit detection degrade gracefully (#315). Registered in `registry.sh` (`AGENT_REGISTERED_PROVIDERS="claude codex"`). Loop integration (routing `response_analyzer.sh` through the dispatch so `ralph --provider codex` runs end-to-end) + docs + E2E are MP.6b.
