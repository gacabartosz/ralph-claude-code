...
10. **lib/agents/** - Multi-provider adapter seam (Issues #312 MP.1, #313 MP.2, #314 MP.3)
    - `registry.sh`: dispatches to the active provider's adapter — `agent_build_command()` (command construction), `agent_detect_format()` / `agent_normalize_response()` (output normalization). `AGENT_PROVIDER` defaults to `claude`; `AGENT_REGISTERED_PROVIDERS` + `agent_provider_is_registered()` back provider validation (#314). Sourced from `ralph_loop.sh` **after** the `_env_AGENT_PROVIDER` capture so the lib's source-time default never masks an explicit environment value (capture-before-source convention).
    - **Provider selection (#314):** `--provider <name>` CLI flag (validated at parse time), resolved in `main()` with precedence **env > `--provider` > `.ralphrc` > default `claude`**; unknown providers exit with guidance. `--help` lists registered providers; `--monitor` forwards `--provider` into the tmux pane (env/.ralphrc are inherited). `.ralphrc` carries `AGENT_PROVIDER` (generated by `lib/enable_core.sh`).
    - **Capabilities matrix (#315, MP.4):** each adapter declares a capability record (`AGENT_CLAUDE_CAPABILITIES` + `agent_claude_has_capability`); the registry exposes `agent_has_capability <cap>` (unknown providers support nothing) and `agent_capability_enabled <cap> [label]`, which returns 0 if supported else logs a **one-time** WARN per capability and returns 1 so callers can no-op the dependent feature. Claude declares the full set (`supports_token_usage`, `supports_permission_denials`, `supports_api_limit_detection`, `supports_session_resume`). **The gates are applied (MP.4b):** `MAX_TOKENS_PER_HOUR` token limiting (`can_make_call`), the permission-denial circuit breaker #101 (`should_exit_gracefully`), API-limit detection #100/#183 (`execute_claude_code`), and session continuity (`main()`) each no-op + warn once when the active provider lacks the capability. Exit detection via `RALPH_STATUS` stays always-on (ungated). Claude supports everything, so these gates are no-ops for the default provider.
    ...
    - `lib/response_analyzer.sh` sources the **registry** and `analyze_response()` routes format detection + normalization through the dispatchers (`agent_detect_format` / `agent_normalize_response`), so it analyzes whichever provider is active (#317 MP.6b). The Claude path is byte-identical (dispatch routes `claude`→`agent_claude_*`). Because the analyzer now pulls in the registry transitively, `ralph_loop.sh` captures `_env_AGENT_PROVIDER` at the very top (before any lib source) to keep the capture-before-source convention intact.
    ...
    - `codex.sh`: the pilot non-Claude adapter (#317, MP.6a) — the template for #318-#322. Build `codex exec [resume <id>] --json [-m <model>] <prompt>` (reuses `CLAUDE_MODEL`/`CLAUDE_USE_CONTINUE`, prepends loop context, never emits `--dangerously-bypass`); normalize maps Codex JSONL events → Ralph's internal struct (RALPH_STATUS in assistant text drives exit detection). Capabilities: `supports_token_usage`, `supports_session_resume` only — so the permission-denial CB and API-limit detection degrade gracefully (#315). Registered in `registry.sh` (`AGENT_REGISTERED_PROVIDERS="claude codex"`). Loop integration is wired (MP.6b): `analyze_response()` routes through the dispatch, so `ralph --provider codex` analyzes Codex JSONL end-to-end with exit detection via `RALPH_STATUS`. Docs + E2E shipped (MP.6c): `docs/providers/CODEX.md`, a README "Multi-Provider Support" capability matrix, an `enable_core.sh` `.ralphrc` note, and `tests/integration/test_codex_e2e.bats` (drives the adapter against a `harness_make_mock_cli` codex stub: build→run→normalize→exit). #317 complete.
    - `gemini.sh`: the second non-Claude adapter (#318, MP.7a). Build `gemini -o json [-m <model>] [--approval-mode <mode>] [--session-id <id> -r] -p <prompt>` — race-free pre-assigned session + resume; never emits `--approval-mode yolo`; reuses `CLAUDE_MODEL`/`CLAUDE_USE_CONTINUE`/`CLAUDE_OUTPUT_FORMAT` plus `GEMINI_CMD`/`GEMINI_APPROVAL_MODE`. Normalize maps Gemini's `-o json` object (`.response`/`.session_id`) → Ralph's struct (RALPH_STATUS drives exit). Capabilities `supports_token_usage`, `supports_session_resume`. Registered (`AGENT_REGISTERED_PROVIDERS="claude codex gemini"`); `ralph --provider gemini` works end-to-end via the analyzer dispatch. Docs + E2E shipped (MP.7b): `docs/providers/GEMINI.md`, a README provider-matrix row, an `enable_core.sh` note, and `tests/integration/test_gemini_e2e.bats` (build→run mock→normalize→exit; pre-assigned session forwarded). #318 complete.
    - `opencode.sh`: third non-Claude adapter (#319, MP.8a). Build `opencode run --format json [-m <provider/model>] [-s <session-id>] <prompt>` (prompt positional; resume via `-s`; never emits `--dangerously-skip-permissions`; reuses `CLAUDE_MODEL`/`CLAUDE_USE_CONTINUE`/`CLAUDE_OUTPUT_FORMAT` + `OPENCODE_CMD`). Normalize maps OpenCode JSONL events (`.sessionID`, assistant `message` text) → Ralph's struct (RALPH_STATUS drives exit). Capabilities `supports_token_usage`, `supports_session_resume`. Registered (`...gemini opencode`); works end-to-end via the analyzer dispatch. Docs + E2E shipped (MP.8b): `docs/providers/OPENCODE.md`, a README provider-matrix row, an `enable_core.sh` note, and `tests/integration/test_opencode_e2e.bats` (build→run mock→normalize→exit; `-s` resume forwarded). #319 complete.
    - `droid.sh`: fourth non-Claude adapter (Factory Droid, #320, MP.9a). Build `droid exec [-o json] [-m <model>] [--session-id <id>] [--auto <level>] <prompt>` (prompt positional; resume via `--session-id`; reuses `CLAUDE_MODEL`/`CLAUDE_USE_CONTINUE`/`CLAUDE_OUTPUT_FORMAT` + `DROID_CMD`/`DROID_AUTO`; omits `-m` when `CLAUDE_MODEL` empty → Droid default claude-opus-4-8). Normalize maps Droid's `-o json` object (`.result`/`.session_id`) → Ralph's struct (RALPH_STATUS drives exit). Capabilities `supports_token_usage`, `supports_session_resume`. Registered (`...opencode droid`); works end-to-end via the analyzer dispatch. Docs + E2E shipped (MP.9b): `docs/providers/DROID.md`, a README provider-matrix row, an `enable_core.sh` note, and `tests/integration/test_droid_e2e.bats` (build→run mock→normalize→exit; `--session-id` resume forwarded). #320 complete.
    - `kilocode.sh`: fifth non-Claude adapter (#321, MP.10a). Build `kilocode --auto [-j] [-mo <model>] [-c] <prompt>` (`--auto` always — headless requirement; `-j` structured output needs `--auto`; model flag is `-mo` not `-m`; `-c` continue-LAST-only when `CLAUDE_USE_CONTINUE=true` + a prior session id exists — no session-id targeting, the weakest resume form; prompt positional with loop context prepended; **never** emits `--yolo` — security invariant since `--auto` already gives headless autonomy; reuses `CLAUDE_MODEL`/`CLAUDE_USE_CONTINUE`/`CLAUDE_OUTPUT_FORMAT` + `KILOCODE_CMD`). Normalize maps Kilocode's `-j` object (`.result`/`.session_id`) → Ralph's struct (RALPH_STATUS drives exit). Capabilities `supports_token_usage`, `supports_session_resume` (continue-last caveat documented). Registered (`...droid kilocode`); works end-to-end via the analyzer dispatch. Docs + E2E shipped (MP.10b): `docs/providers/KILOCODE.md`, a README provider-matrix row (continue-last† footnote), an `enable_core.sh` note, and `tests/integration/test_kilocode_e2e.bats` (build→run mock→normalize→exit; `-c` continue-last forwarded). #321 complete.
    - `copilot.sh`: sixth non-Claude adapter — the **DEGRADED, TEXT-ONLY** case (GitHub Copilot CLI, #322, MP.11a). Build `copilot -s [--model <m>] [--resume <id>] [--allow-tool <t>]… [--deny-tool <t>]… -p <prompt>` (`-s/--silent` always; prompt via `-p` flag with loop context prepended; `--model` from shared `CLAUDE_MODEL`; `--resume <id>` resume-by-id; granular `--allow-tool`/`--deny-tool` from `COPILOT_ALLOWED_TOOLS`/`COPILOT_DENIED_TOOLS`; **never** emits `--allow-all` — security invariant; `CLAUDE_OUTPUT_FORMAT` ignored — Copilot has no JSON). `agent_copilot_detect_format` **always returns "text"**, so `analyze_response` takes its RALPH_STATUS text-mode path (#224) — exit detection is fully portable without structured output; `agent_copilot_normalize_response` parses TEXT → struct. Capabilities **`supports_session_resume` ONLY** (no token_usage/permission_denials/api_limit → all degrade per #315 — the hardest test of the degradation design). Registered (`...kilocode copilot`). Docs + E2E shipped (MP.11b): `docs/providers/COPILOT.md`, a README provider-matrix row (text-only‡), an `enable_core.sh` note, and `tests/integration/test_copilot_e2e.bats` (build→run mock→detect=text→`analyze_response` exit via RALPH_STATUS; `--resume` forwarded; degraded gates warn-and-no-op). #322 complete.
    - **Provider surfacing (#324, MP.13):** `update_status()` writes `"provider": "${AGENT_PROVIDER:-claude}"` into `.ralph/status.json`, and `ralph_monitor.sh` `display_status()` reads `.provider // "claude"` and renders a `Provider:` line in the dashboard "Current Status" box (graceful fallback for older/corrupt status files). **Bats:** `test_status_updates.bats` (status field) + `tests/integration/test_monitor.bats` (monitor render).
