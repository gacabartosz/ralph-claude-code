...
10. **lib/agents/** - Multi-provider adapter seam (Issues #312 MP.1, #313 MP.2, #314 MP.3)
    ...
    - **Capabilities matrix (#315, MP.4a):** each adapter declares a capability record (`AGENT_CLAUDE_CAPABILITIES` + `agent_claude_has_capability`); the registry exposes `agent_has_capability <cap>` (unknown providers support nothing) and `agent_capability_enabled <cap> [label]`, which returns 0 if supported else logs a **one-time** WARN per capability and returns 1 so callers can no-op the dependent feature. Claude declares the full set (`supports_token_usage`, `supports_permission_denials`, `supports_api_limit_detection`, `supports_session_resume`). Applying these gates to the feature sites (token limiting, permission-denial CB #101, API-limit detection #100/#183, session resume) is MP.4b; exit detection via `RALPH_STATUS` stays always-on.
