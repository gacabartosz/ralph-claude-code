...
- [ ] MP.4b — `#315` P2.1 APPLY the capability gates in `ralph_loop.sh` (Claude=all-true ⇒ no behavior change today): gate `MAX_TOKENS_PER_HOUR` token limiting on `supports_token_usage`; the permission-denial circuit breaker (#101, ~line 694) on `supports_permission_denials`; API-limit detection (#100/#183, ~line 1951) on `supports_api_limit_detection`; session resume/`--resume` on `supports_session_resume`. Exit detection via `RALPH_STATUS` stays always-on. **Bats:** degradation-path tests (each gate off → feature no-ops + warns once; on → unchanged).
